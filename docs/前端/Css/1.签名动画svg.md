...
若有钢笔工具画的`一笔画签名`，则可直接跳过`第一步`

若没有签名的svg，则可按下面的方式生成

- 生成签名图片：https://fontmeme.com/zh/signature-fonts/ 也可以选择其他（png图片都可以）

- 转化为svg：https://imagetostl.com/cn/convert/file/png/to/svg#convert
...
添加 id=`logo-anim` 或其他方式映射该样式

![添加id](assets/1.签名动画svg/image.png)
